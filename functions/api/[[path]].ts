...
function getCanonicalCycleStatus(db: any): any {
  ...
  const currentDay = Math.min(
    baseDays + (activeCycle.extendedDays || 0),
  ...
  return {
    ...
    endDate:
      activeCycle.endDate ||
      new Date(startMs + baseDurationSeconds * 1000).toISOString(),
  ...
  if (activeCycle && canonical.isActive) {
    ...
    if (
      ...
      writeAuditLog(
        ...
        "SYSTEM",
        "SYSTEM",
        "CYCLE_COMPLETED_AUTO",
        null,
        `Operating cycle ${activeCycle.id} ended. Revenue: ₦${totalRevenue}, Expenses: ₦${totalExpenses}, Net: ₦${netGeneratedAmount}, Pool: ₦${distributionPool}`,
        db,
      );
      dbChanged = true;
    }
...
function calculateInstallmentsForDriver(
  driver: any,
  db: any,
  activeCycle: any,
) {
...
function isOperationalCycleNotification(n: any): boolean {
  ...
  const text =
    `${n.title_en || n.titleEn || n.title || ""} ${n.message_en || n.messageEn || n.message || ""} ${n.title_ha || n.titleHa || ""} ${n.message_ha || n.messageHa || ""}`.toLowerCase();
...
function isNotificationForUser(
  ...
  if (user.role === "driver") {
    const drv = user.id
      ? db.drivers?.find((d: any) => d.user_id === user.id || d.id === user.id)
      : null;
  ...
  if (user.role === "shareholder") {
    const sh = user.id
      ? db.shareholders?.find(
          (s: any) => s.user_id === user.id || s.id === user.id,
        )
      : null;
    ...
    if (!n.user_id && !n.target_role) {
      ...
      const text =
        `${n.title_en || n.titleEn || n.title || ""} ${n.message_en || n.messageEn || n.message || ""}`.toLowerCase();
      if (
  ...
  if (user.role === "director" || user.role === "admin") {
    if (
      !n.user_id ||
      (user.id && n.user_id === user.id) ||
      !n.target_role ||
      n.target_role === user.role ||
      n.target_role === "all"
    )
      return true;
  }
...
function generateFilteredPayload(
  ...
  if (role === "director") {
    ...
    return {
      ...
      users: db.users || [],
      admins: db.admins || [],
      shareholders: (db.shareholders || []).map((s: any) => ({
        id: s.id,
    ...
    const totalInvested = (db.shareholders || []).reduce(
      (sum: number, s: any) => sum + (parseFloat(s.investment_amount) || 0),
      0,
    );
    const cleanShareholders = (db.shareholders || []).map((s: any) => {
      if (s.id === shareholderId) {
        const equityPercentage =
          totalInvested > 0
            ? (
                ((parseFloat(s.investment_amount) || 0) / totalInvested) *
                100
              ).toFixed(2)
            : "0";
    ...
    return {
      ...
      notifications: (db.notifications || []).filter((n: any) =>
        isNotificationForUser(
          n,
          { id: shareholderId, role: "shareholder" },
          db,
        ),
      ),
    };
    ...
    const driverNotifications = (db.notifications || []).filter((n: any) =>
      isNotificationForUser(
        n,
        { id: activeDriver.user_id || driverProfileId, role: "driver" },
        db,
      ),
    );
...
async function sendPushForNotification(env: Env, db: any, n: any) {
  try {
    ...
    const payload = JSON.stringify({
      ...
      messageEn:
        enriched.messageEn || enriched.message_en || n.message_en || "",
      messageHa:
    ...
    if (n.user_id) {
      ...
      const targetRoleObj = db.roles?.find(
        (r: any) => r.name.toLowerCase() === roleName,
      );
      if (targetRoleObj) {
    ...
    if (isBroadcast) {
      ...
      const allowedRoles = isCycleNotif
        ? ["director", "admin", "driver", "shareholder"]
        : ["director", "admin"];
      await sendPushNotificationToUserOrRole(
        env,
        { roles: allowedRoles },
        { title: enriched.titleEn, message: enriched.messageEn, type: n.type },
        db,
      );
    } else {
      for (const uid of targetUserIds) {
        ...
        if (prefs && prefs.enablePush === false) {
          console.log(
            `PushService Worker: Skipping push for user ${uid} due to preference.`,
          );
          continue;
        }
        await sendPushNotificationToUserOrRole(
          ...
          { userId: uid },
          {
            title: enriched.titleEn,
            message: enriched.messageEn,
            type: n.type,
          },
          db,
        );
...
const FIREBASE_CONFIG = {
  ...
  firestoreDatabaseId:
    "ai-studio-ruqayyatransport-ec9c3d70-1fac-4a98-a67d-8c340e7f6358",
};
...
async function fetchFromFirestore(): Promise<any> {
  try {
    ...
    if (!res.ok) {
      if (res.status === 404) {
        console.log(
          "[FIRESTORE REST] main_database document not found (404), starting fresh.",
        );
        return null;
      }
    ...
    if (doc && doc.fields) {
      ...
      console.log(
        "[FIRESTORE REST] Successfully loaded database state from Firestore REST API.",
      );
      return plain;
    }
    return null;
    ...
    console.error(
      "[FIRESTORE REST ERROR] Failed to load database state from Firestore:",
      err.message,
    );
    return null;
  }
...
async function saveFileToFirestore(
  ...
  try {
    ...
    const converted = plainToFirestore({
      base64: cleanBase64,
      timestamp: new Date().toISOString(),
    });
    const fields =
      converted && converted.mapValue ? converted.mapValue.fields : {};
    const body = { fields };
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `HTTP error! status: ${res.status}, response: ${errText}`,
      );
    }
    console.log(
      `[FIRESTORE FILE REST] Successfully saved file ${filename} to Firestore.`,
    );
  } catch (err: any) {
    console.error(
      `[FIRESTORE FILE REST ERROR] Failed to save file ${filename} to Firestore:`,
      err.message,
    );
  }
}

async function fetchFileFromFirestore(
  filename: string,
): Promise<string | null> {
  try {
    ...
    if (!res.ok) {
      if (res.status === 404) {
        console.log(
          `[FIRESTORE FILE REST] File ${filename} not found in Firestore.`,
        );
        return null;
      }
      throw new Error(`HTTP error! status: ${res.status}`);
    ...
    console.error(
      `[FIRESTORE FILE REST ERROR] Failed to load file ${filename} from Firestore:`,
      err.message,
    );
    return null;
  }
...
async function saveToFirestore(state: any): Promise<void> {
  try {
    ...
    console.log(
      "[FIRESTORE REST] Successfully saved database state to Firestore REST API.",
    );
  } catch (err: any) {
    console.error(
      "[FIRESTORE REST ERROR] Failed to save database state to Firestore:",
      err.message,
    );
  }
}
  ...
  async getDB(): Promise<any> {
    ...
    try {
      if (d1) {
        ...
        await d1.batch([
          d1.prepare(
            `CREATE TABLE IF NOT EXISTS collections (name TEXT PRIMARY KEY, data TEXT)`,
          ),
          d1.prepare(
            `CREATE TABLE IF NOT EXISTS users (id TEXT PRIMARY KEY, name TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`,
          ),
          d1.prepare(
            `CREATE TABLE IF NOT EXISTS cycles (id TEXT PRIMARY KEY, user_id TEXT, start_time DATETIME, end_time DATETIME, duration INTEGER, status TEXT)`,
          ),
          d1.prepare(
            `CREATE TABLE IF NOT EXISTS subscriptions (id TEXT PRIMARY KEY, user_id TEXT, endpoint TEXT UNIQUE, keys TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`,
          ),
        ]);

        const dbResponse = await d1
          .prepare("SELECT name, data FROM collections")
          .all();
        const results =
          dbResponse?.results ||
          (Array.isArray(dbResponse) ? dbResponse : null);
        ...
        if (results && results.length > 0) {
          ...
          console.log(
            `[D1 SQL DB SUCCESS] Loaded database state in ${Date.now() - startTime}ms`,
          );
        } else {
          console.log(
            `[D1 SQL DB SEED] No collections found, preparing seed defaults...`,
          );
          const seedState = await this.ensureDefaults({});
          if (seedState && seedState.notifications) {
            this.loadedNotificationIds = new Set(
              seedState.notifications.map((n: any) => n.id).filter(Boolean),
            );
          }
        ...
        console.log(
          `[D1 Fallback] No D1 DB bound. Attempting to fetch from Firestore REST API...`,
        );
        ...
        if (firestoreDb) {
          ...
          if (!this.memoryDb) {
            console.log(
              `[MEMORY DB QUERY] No persistent DB or Firestore found, initializing Memory fallback...`,
            );
      ...
      if (db && db.notifications) {
        this.loadedNotificationIds = new Set(
          db.notifications.map((n: any) => n.id).filter(Boolean),
        );
      }
      this.dbCache = db;
      return db;
      ...
      console.error(
        `[DB RESOLUTION ERROR] D1 query failed, falling back to Firestore/Memory:`,
        dbError,
      );
      try {
        const firestoreDb = await fetchFromFirestore();
        ...
        console.error(
          `[FALLBACK ERROR] Firestore fallback also failed:`,
          fallbackErr,
        );
        throw dbError;
      }
  ...
  async saveDB(state: any): Promise<void> {
    if (state) {
      ...
      if (state.company_settings.wallet_initial_amount === undefined) {
        state.company_settings.wallet_initial_amount =
          state.company_settings.wallet_balance !== undefined
            ? state.company_settings.wallet_balance
            : 0;
      }
      const totalRev = (state.financial_records || [])
        .filter((f: any) => f.type === "revenue" || f.type === "deposit")
        .reduce((sum: number, f: any) => sum + (parseFloat(f.amount) || 0), 0);
      const totalExp = (state.financial_records || [])
        .filter((f: any) => f.type === "expense" || f.type === "withdrawal")
        .reduce((sum: number, f: any) => sum + (parseFloat(f.amount) || 0), 0);
      state.company_settings.wallet_balance =
        (parseFloat(state.company_settings.wallet_initial_amount) || 0) +
        totalRev -
        totalExp;
    }
    ...
    if (state && state.notifications) {
      ...
      for (const n of newNotifications) {
        ...
        await sendPushForNotification(this.env, state, n).catch((err: any) => {
    ...
    if (d1) {
      ...
      if (
        ...
        for (const c of state.cycles) {
          statements.push(
            d1
              .prepare(
                "INSERT OR REPLACE INTO cycles (id, user_id, start_time, end_time, duration, status) VALUES (?, ?, ?, ?, ?, ?)",
              )
              .bind(
                c.id,
                c.created_by || null,
      ...
      if (
        updatedKeys.includes("push_subscriptions") &&
        state.push_subscriptions &&
        Array.isArray(state.push_subscriptions)
      ) {
        for (const s of state.push_subscriptions) {
          if (s && s.subscription && s.subscription.endpoint) {
            statements.push(
              d1
                .prepare(
                  "INSERT OR REPLACE INTO subscriptions (id, user_id, endpoint, keys, created_at) VALUES (?, ?, ?, ?, ?)",
                )
                .bind(
      ...
      if (statements.length > 0) {
        console.log(
          `[D1 SQL DB SAVE] Executing batch update for ${statements.length} statements. Collections: ${updatedKeys.join(", ")}`,
        );
  ...
  private async ensureDefaults(parsed: any): Promise<any> {
      ...
      parsed.roles = [
        {
          id: "role-director",
          name: "director",
          description: "Executive Boardroom Director",
          created_at: new Date().toISOString(),
          status: "active",
        },
        {
          id: "role-admin",
          name: "admin",
          description: "Operations Control Center Admin",
          created_at: new Date().toISOString(),
          status: "active",
        },
        {
          id: "role-driver",
          name: "driver",
          description: "Logistics Fleet Driver",
          created_at: new Date().toISOString(),
          status: "active",
        },
        {
          id: "role-shareholder",
          name: "shareholder",
          description: "Corporate Capital Investor",
          created_at: new Date().toISOString(),
          status: "active",
        },
      ];
    ...
    if (!parsed.permissions || parsed.permissions.length === 0) {
      parsed.permissions = [
        {
          id: "p1",
          name: "view_director_dashboard",
          description: "View executive metrics",
        },
        {
          id: "p2",
          name: "approve_drivers",
          description: "Approve or reject new driver registrations",
        },
        {
          id: "p3",
          name: "manage_financials",
          description: "Access and modify general ledger records",
        },
        {
          id: "p4",
          name: "approve_vouchers",
          description: "Approve driver fuel allocation vouchers",
        },
        {
          id: "p5",
          name: "view_audit_logs",
          description: "Inspect corporate security records",
        },
        {
          id: "p6",
          name: "request_vouchers",
          description: "Submit fuel purchase requests",
        },
      ];
      changed = true;
    }
    ...
    if (!parsed.users || parsed.users.length === 0) {
      ...
      parsed.admins = [
        ...
        {
          id: generateUUID(),
          user_id: abakakaAdminUserId,
          company_id: "ADM-2026-002",
          passport_photo_url: "",
          created_at: new Date().toISOString(),
          status: "active",
        },
      ...
      parsed.notifications = [
        {
          ...
          message_en:
            "Welcome to Ruqayya Transport Limited ERP. Secure database clusters configured.",
          message_ha:
            "Barka da zuwa Ruqayya Transport Limited ERP. An tsara rumbun adana bayanai lafiya.",
          type: "success",
          read_status: 0,
          created_at: new Date().toISOString(),
        },
    ...
    if (!parsed.financial_records) {
      parsed.financial_records = [];
      changed = true;
    }
    if (!parsed.trip_manifests) {
      parsed.trip_manifests = [];
      changed = true;
    }
    if (!parsed.cycles) {
    ...
    if (!parsed.driver_payments) {
      parsed.driver_payments = [];
      changed = true;
    }
    if (!parsed.vehicle_documents) {
      parsed.vehicle_documents = [];
      changed = true;
    }
    if (!parsed.driver_documents) {
      parsed.driver_documents = [];
      changed = true;
    }
    if (!parsed.company_documents) {
      parsed.company_documents = [];
      changed = true;
    }
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        console.log(
          `[PUSH] Successfully encrypted payload with RFC 8188 header for endpoint: ${endpoint}`,
        );
      } catch (encryptErr) {
        console.warn(
          `[PUSH] Encryption failed, falling back to plaintext (unlikely to work in prod):`,
          encryptErr,
        );
      }
    }
    ...
    if (!res.ok && body !== null) {
      console.warn(
        `Push service rejected payload (${res.status}). Retrying with silent push...`,
      );
...
async function sendPushNotificationToUserOrRole(
    ...
    try {
      ...
      for (const keyInfo of keys) {
        ...
        if (target.all) {
          ...
          if (
            userRole &&
            target.roles.map((r) => r.toLowerCase()).includes(userRole)
          ) {
            shouldSend = true;
          }
        } else if (target.role) {
        ...
        if (shouldSend) {
          const subscriptionJson = await env.PUSH_SUBSCRIPTIONS.get(
            keyInfo.name,
          );
          if (subscriptionJson) {
  ...
  if (db && db.push_subscriptions && Array.isArray(db.push_subscriptions)) {
    db.push_subscriptions.forEach((entry: any) => {
      if (entry && entry.subscription && entry.subscription.endpoint) {
        ...
        const alreadyExists = subscriptionsToNotify.some(
          (item) => item.subscription.endpoint === entry.subscription.endpoint,
        );
        if (!alreadyExists) {
          let shouldSend = false;
          if (target.all) {
  ...
  if (subscriptionsToNotify.length === 0) {
    console.log(
      "No registered push subscriptions matched this notification's target audience.",
    );
    return;
  }
  ...
  for (const item of subscriptionsToNotify) {
    try {
        ...
        if (db && db.push_subscriptions) {
          ...
          db.push_subscriptions = db.push_subscriptions.filter(
            (s: any) =>
              s &&
              s.subscription &&
              s.subscription.endpoint !== item.subscription.endpoint,
          );
      ...
      console.error(
        `Error processing subscription dispatch for endpoint ${item.subscription.endpoint}:`,
        err,
      );
    }
  }
  ...
  if (dbChanged && db) {
    ...
    console.log(
      "D1 DB push_subscriptions collection pruned due to expired subscriptions.",
    );
  }
}
...
export const onRequest: PagesFunction<Env> = async (context) => {
  try {
    ...
    const authenticate = async () => {
      ...
      if (!token) {
        return {
          authenticated: false,
          error:
            "Authentication required. Active session parameters not found.",
          status: 412,
        };
      }

      let session = db.sessions.find(
        (s: any) => s.token === token && s.status === "active",
      );
        ...
        if (token.startsWith("tok_")) {
          ...
          if (roleName && userKey) {
            ...
            if (user) {
              if (
                userKey === "ADAM" &&
                (!user.full_name || user.full_name.includes("Ibrahim"))
              ) {
            ...
            if (!session) {
              ...
              session = {
                ...
                user_ip:
                  ...
                  "127.0.0.1",
                user_agent:
                  request.headers.get("user-agent") || "Corporate API Consumer",
                created_at: new Date().toISOString(),
                status: "active",
              };
      ...
      if (new Date(session.expires_at) < new Date()) {
        ...
        return {
          authenticated: false,
          error: "Your corporate session has expired.",
          status: 401,
        };
      }
      ...
      if (!user) {
        return {
          authenticated: false,
          error: "Associated user record not found.",
          status: 401,
        };
      }
      ...
      const role =
        db.roles.find((r: any) => r.id === user.role_id)?.name || "public";
      return {
        authenticated: true,
        user: {
          id: user.id,
    ...
    // Handle Cloudflare Pages Functions WebSockets
    if (path === "/api/ws/driver-location") {
      const upgradeHeader = request.headers.get("Upgrade");
      if (upgradeHeader !== "websocket") {
        return buildResponse({ error: "Expected Upgrade: websocket" }, 426);
      }
      ...
      server.addEventListener("message", async (event: any) => {
        try {
          ...
          const targetDriverId = payload.driverId || driverId;
          ...
          if (!isNaN(lat) && !isNaN(lng) && targetDriverId) {
            ...
            server.send(
              JSON.stringify({ type: "location_ack", success: true }),
            );
          }
        } catch (err: any) {
          console.error("[CF WS Error]", err.message);
        }
      });
    ...
    if (path === "/api/notifications/vapid-public-key" && method === "GET") {
      ...
      if (!keys || !keys.publicKey) {
        return buildResponse(
          { error: "VAPID keys not configured or generation failed." },
          500,
        );
      }
    ...
    if (path === "/api/notifications/subscribe" && method === "POST") {
      try {
        ...
        if (env.PUSH_SUBSCRIPTIONS) {
          ...
          console.warn(
            "PUSH_SUBSCRIPTIONS KV binding is missing. Persisted in D1 DB only.",
          );
          return buildResponse({
            success: true,
    ...
    if (path === "/api/auth/register-director" && method === "POST") {
      try {
        ...
        if (hasExistingDirectors) {
          ...
          if (!session) {
            return buildResponse(
              { error: "Invalid executive session token." },
              401,
            );
          }
        ...
        if (
          ...
          return buildResponse(
            { error: "Email already mapped to an active ERP credential." },
            400,
          );
        }
        ...
        if (passportPhoto) {
          ...
          if (env.R2_BUCKET) {
            try {
              ...
              console.error(
                `[R2 ERROR] Failed to upload director photo:`,
                r2Err,
              );
            }
          }
        ...
        writeAuditLog(
          ...
          "DIRECTOR_CREATION",
          null,
          `Created Director User: ${fullName} (${companyId})`,
          db,
        );
      ...
      try {
        ...
        if (emailExists) {
          return buildResponse(
            {
              error:
                "This email address is already registered inside our fleet.",
            },
            400,
          );
        }
        ...
        const plateExists = db.vehicles.some(
          (v: any) =>
            v.plate_number.toUpperCase() === vehicle.plateNumber.toUpperCase(),
        );
        if (plateExists) {
          return buildResponse(
        ...
        if (personal.passportPhoto) {
          ...
          if (env.R2_BUCKET) {
            const cleanBase64 = personal.passportPhoto.replace(
              /^data:.*?;base64,/,
              "",
            );
            const binaryString = atob(cleanBase64);
        ...
        if (guarantor.passport) {
          ...
          if (env.R2_BUCKET) {
            const cleanBase64 = guarantor.passport.replace(
              /^data:.*?;base64,/,
              "",
            );
            const binaryString = atob(cleanBase64);
        ...
        const newDriver = {
          ...
          company_driver_id:
            ...
            `PEND-${generateUUID().substring(0, 4).toUpperCase()}`,
          companyDriverId:
            personal.companyDriverId ||
            `PEND-${generateUUID().substring(0, 4).toUpperCase()}`,
          full_name: personal.fullName,
        ...
        writeAuditLog(
          ...
          "SELF_REGISTRATION",
          null,
          `Driver registered self under UUID ${driverId}`,
          db,
        );
        await dbManager.saveDB(db);
      ...
      try {
        ...
        if (username) {
          ...
          if (user) {
            ...
            return buildResponse(
              { error: "Access Denied: Invalid enterprise username." },
              401,
            );
          }
          ...
          if (!email || !password) {
            return buildResponse(
              {
                error:
                  "Please enter both corporate email and security password.",
              },
              400,
            );
          }
          ...
          if (!user) {
            return buildResponse(
              { error: "Access Denied: Non-existent active user profile." },
              401,
            );
          }
        ...
        writeAuditLog(
          ...
          "SESSION_CREATED",
          null,
          `Authorized login session valid until ${expiresAt}`,
          db,
        );
        await dbManager.saveDB(db);
    ...
    if (path === "/api/auth/login-as-role" && method === "POST") {
      try {
        ...
        if (!targetUser) {
          return buildResponse(
            { error: `Pre-seeded user profile not found for role ${role}.` },
            404,
          );
        }
        ...
        const userKey = targetUser.username
          ? targetUser.username.toUpperCase()
          : targetUser.email.split("@")[0].toUpperCase();
        ...
        writeAuditLog(
          ...
          "DEMO_SWITCH_LOGIN",
          null,
          `Logged into preview account successfully`,
          db,
        );
        await dbManager.saveDB(db);
    ...
    if (path === "/api/auth/me" && method === "GET") {
      ...
      const permissions = db.permissions
        .filter((p: any) => {
          ...
          if (user.role === "admin" && p.name !== "view_audit_logs")
            return true;
          if (user.role === "driver" && p.name === "request_vouchers")
            return true;
          return false;
        })
        .map((p: any) => p.name);
          ...
          const documents = (db.driver_documents || [])
            ...
            .map((doc: any) => ({
              ...
              file_url: doc.file_url
                ? doc.file_url.includes("token=")
                  ? doc.file_url
                  : `${doc.file_url}?token=${encodeURIComponent(token)}`
                : "",
            }));
          const passportDoc = documents.find(
            (doc: any) => doc.document_type === "passport_photo",
          );
          const passport_photo_url =
            userRec.passport_photo_url ||
            (passportDoc ? passportDoc.file_url : "");
          ...
          profileDetails = {
            ...
            guarantor: guarantor
              ? {
                  ...
                  nin: guarantor.nin || "",
                  passport:
                    guarantor.passport || guarantor.passport_photo_url
                      ? (
                          guarantor.passport || guarantor.passport_photo_url
                        ).includes("token=")
                        ? guarantor.passport || guarantor.passport_photo_url
                        : `${guarantor.passport || guarantor.passport_photo_url}?token=${encodeURIComponent(token)}`
                      : "",
                  passportPhotoUrl:
                    ...
                    guarantor.passport
                      ? (
                        ...
                        ? guarantor.passport_photo_url ||
                          guarantor.passportPhotoUrl ||
                          guarantor.passport
                        : `${guarantor.passport_photo_url || guarantor.passportPhotoUrl || guarantor.passport}?token=${encodeURIComponent(token)}`
                      : "",
                  passport_photo_url:
                    guarantor.passport_photo_url ||
                    guarantor.passportPhotoUrl ||
            ...
            vehicle: vehicle
              ? {
                  ...
                  registration_number:
                    ...
                    "",
                  chassisNumber:
                    vehicle.chassis_number || vehicle.chassisNumber || "",
                  chassis_number:
                    vehicle.chassis_number || vehicle.chassisNumber || "",
                  engineNumber:
                    vehicle.engine_number || vehicle.engineNumber || "",
                  engine_number:
                    vehicle.engine_number || vehicle.engineNumber || "",
    ...
    if (path === "/api/auth/me" && method === "PUT") {
      try {
        ...
        let photo =
          payload.passportPhoto || payload.passport_photo_url || payload.avatar;
          ...
          if (
            ...
            if (env.R2_BUCKET) {
              try {
                ...
                console.error(
                  `[R2 ERROR] Failed to upload updated avatar to bucket:`,
                  r2Err,
                );
              }
            }
    ...
    if (path === "/api/auth/logout" && method === "POST") {
      ...
      writeAuditLog(
        ...
        "SESSION_DESTROYED",
        null,
        "Manual logout request processed",
        db,
      );
      await dbManager.saveDB(db);
    ...
    if (path === "/api/audit-logs" && method === "GET") {
      if (user.role !== "director" && user.role !== "admin") {
        return buildResponse(
          {
            error:
              "Access Denied: Director or Admin role required for audit trails.",
          },
          403,
        );
      }
    ...
    if (path.startsWith("/api/drivers")) {
      ...
      if (parts.length === 0 && method === "GET") {
        ...
        const results = list.map((drv: any) => {
          ...
          return {
            ...
            company_driver_id:
              drv.company_driver_id || drv.companyDriverId || "",
            guarantor: g
              ? {
                  ...g,
                  ...
                  nin: g.nin || "",
                  passport:
                    g.passport || g.passport_photo_url
                      ? (g.passport || g.passport_photo_url).includes("token=")
                        ? g.passport || g.passport_photo_url
                        : `${g.passport || g.passport_photo_url}?token=${encodeURIComponent(token)}`
                      : "",
                  passportPhotoUrl:
                    g.passport_photo_url || g.passportPhotoUrl || g.passport
                      ? (
                        ...
                        ? g.passport_photo_url ||
                          g.passportPhotoUrl ||
                          g.passport
                        : `${g.passport_photo_url || g.passportPhotoUrl || g.passport}?token=${encodeURIComponent(token)}`
                      : "",
                  passport_photo_url:
                    g.passport_photo_url || g.passportPhotoUrl || g.passport
                      ? (
            ...
            vehicle: v
              ...
              : null,
            agreed_amount:
              drv.agreed_amount ?? drv.agreedAmount ?? financials.agreedAmount,
            agreedAmount:
              drv.agreed_amount ?? drv.agreedAmount ?? financials.agreedAmount,
      ...
      if (parts[0] === "import" && method === "POST") {
        ...
        try {
          const { personal, guarantor, vehicle } =
            (await request.json()) as any;
          ...
          const newDriver = {
            ...
            company_driver_id:
              personal.companyDriverId ||
              `RTL-${generateUUID().substring(0, 4).toUpperCase()}`,
            address: personal.address,
            nin: personal.nin,
            ...
            agreedAmount: !isNaN(parseFloat(personal.agreedAmount))
              ? parseFloat(personal.agreedAmount)
              : 0,
            vehicle_purchase_price: !isNaN(
              parseFloat(personal.vehiclePurchasePrice),
            )
              ? parseFloat(personal.vehiclePurchasePrice)
              : 0,
            vehiclePurchasePrice: !isNaN(
              parseFloat(personal.vehiclePurchasePrice),
            )
              ? parseFloat(personal.vehiclePurchasePrice)
              : 0,
            remaining_vehicle_balance: !isNaN(
              parseFloat(personal.remainingVehicleBalance),
            )
              ? parseFloat(personal.remainingVehicleBalance)
              : 0,
            remainingVehicleBalance: !isNaN(
              parseFloat(personal.remainingVehicleBalance),
            )
              ? parseFloat(personal.remainingVehicleBalance)
              : 0,
            opening_balance: {
              is_imported: true,
              total_paid_to_date: !isNaN(parseFloat(personal.totalPaidToDate))
                ? parseFloat(personal.totalPaidToDate)
                : 0,
              remaining_vehicle_balance: !isNaN(
                parseFloat(personal.remainingVehicleBalance),
              )
          ...
          writeAuditLog(
            ...
            user.email,
            user.role,
            "DRIVER_IMPORTED_SUCCESS",
            null,
            `Imported driver ${personal.fullName} with opening balance`,
            db,
          );
          await dbManager.saveDB(db);
          ...
          return buildResponse({
            success: true,
            message: "Driver imported successfully.",
          });
        } catch (err: any) {
          return buildResponse({ error: err.message }, 500);
        }
      ...
      if (parts[0] === "self" && parts[1] === "documents" && method === "GET") {
        ...
        if (!activeDriver)
          return buildResponse({ error: "Driver profile missing." }, 404);

        const docs = db.driver_documents.filter(
          (d: any) => d.driver_id === activeDriver.id,
        );
      ...
      if (parts.length === 1) {
        ...
        let drv = db.drivers.find(
          (d: any) => d.id === targetDriverId || d.user_id === targetDriverId,
        );
        if (
          !drv &&
          (user.role === "driver" ||
          ...
          drv = db.drivers.find(
            (d: any) => d.user_id === user.id || d.id === user.id,
          );
        }
        if (!drv)
        ...
        if (method === "PUT") {
          ...
          try {
            ...
            if (
              ...
              drv.vehicle_purchase_price = parseFloat(
                payload.vehiclePurchasePrice,
              );
              drv.vehiclePurchasePrice = parseFloat(
                payload.vehiclePurchasePrice,
              );
            }
            if (
            ...
            writeAuditLog(
              ...
              null,
              `Admin updated driver profile ${drv.id}`,
              db,
            );
            await dbManager.saveDB(db);
      ...
      if (parts.length === 2 && parts[1] === "archive" && method === "PUT") {
        try {
          ...
          writeAuditLog(
            ...
            "DRIVER_ARCHIVED",
            parts[0],
            `Archived driver ${drv.full_name || drv.fullName}`,
            db,
          );
          await dbManager.saveDB(db);
      ...
      if (parts.length === 2 && parts[1] === "restore" && method === "PUT") {
        try {
          ...
          writeAuditLog(
            ...
            "DRIVER_RESTORED",
            parts[0],
            `Restored driver ${drv.full_name || drv.fullName}`,
            db,
          );
          await dbManager.saveDB(db);
      ...
      if (parts.length === 2 && parts[1] === "status" && method === "PUT") {
        if (user.role !== "admin" && user.role !== "director")
          return buildResponse({ error: "Access Denied." }, 403);
        try {
          const { status, remarks, companyDriverId } =
            (await request.json()) as any;
          const drv = db.drivers.find((d: any) => d.id === parts[0]);
          if (!drv)
            return buildResponse({ error: "Driver profile not found." }, 404);
          ...
          writeAuditLog(
            ...
            null,
            `Driver status adjusted to ${status}. Details: ${remarks || ""}`,
            db,
          );
          await dbManager.saveDB(db);
      ...
      if (parts.length === 2 && parts[1] === "classify" && method === "PUT") {
        ...
        try {
          ...
          writeAuditLog(
            ...
            user.email,
            user.role,
            "DRIVER_CLASSIFY_SHIFT",
            null,
            `Driver classification changed to ${classification}`,
            db,
          );
          await dbManager.saveDB(db);
      ...
      if (
        ...
        const drv = db.drivers.find(
          (d: any) =>
            d.id === targetId ||
            d.user_id === targetId ||
            (targetId === "me" && d.user_id === user.id) ||
            (targetId === "self" && d.user_id === user.id) ||
            (user.role === "driver" && d.user_id === user.id),
        );
        if (!drv)
          return buildResponse({ error: "Driver profile not found." }, 404);
    ...
    // 8.5. DRIVER TELEMATICS & DUTY SHIFTS (SINGULAR DRIVER PREFIX)
    if (path.startsWith("/api/driver")) {
      ...
      const getMaiduguriSimulatedDataCF = (drvObj: any) => {
        const driverNum =
          parseInt(
      ...
      if (
        ...
        if (!drv)
          return buildResponse({ error: "Driver profile not linked." }, 404);
        ...
        const nowIso = new Date().toISOString();
        db.driver_duty_sessions.forEach((s: any) => {
        ...
        const newDutySession = {
          ...
          driver_id: drv.id,
          driver_name: user.full_name || drv.full_name || "Driver",
          company_driver_id: drv.company_driver_id || "DRV-UNKNOWN",
          start_time: nowIso,
          finish_time: null,
          status: "active",
        ...
        db.drivers.forEach((d: any) => {
          if (d.id === drv.id) d.status = "on-duty";
        });
      ...
      if (
        ...
        const activeSession = db.driver_duty_sessions.find(
          (s: any) => s.driver_id === drv.id && s.status === "active",
        );
        const nowIso = new Date().toISOString();
        ...
        if (activeSession) {
          ...
          activeSession.finishing_mileage =
            parseFloat(finishingMileage) || activeSession.starting_mileage;
          activeSession.ending_location =
            endingLocation || placeName || "Maiduguri Central Terminal";

          const lastPlace =
            activeSession.places_visited[
              activeSession.places_visited.length - 1
            ];
          if (lastPlace && lastPlace.status === "active_dwell") {
            lastPlace.status = "completed";
            lastPlace.departed_at = nowIso;
            const dwell = Math.round(
              (new Date(nowIso).getTime() -
                new Date(lastPlace.arrived_at).getTime()) /
                60000,
            );
            lastPlace.dwell_duration_minutes = dwell > 0 ? dwell : 0;
          }

          const terminalRecord = {
            id: `PLC-${Date.now()}-${generateUUID().substring(0, 4)}`,
            place_name:
              endingLocation || placeName || "Maiduguri Central Depot",
            arrived_at: nowIso,
            departed_at: nowIso,
            dwell_duration_minutes: 10,
            status: "completed",
            activity: "Shift Termination & Final Post-trip Log",
            latitude: latitude || 11.8311,
            longitude: longitude || 13.1509,
          };
          activeSession.places_visited.push(terminalRecord);
        }

        db.drivers.forEach((d: any) => {
          if (d.id === drv.id) d.status = "off-duty";
        });
      ...
      if (parts.length === 1 && parts[0] === "location" && method === "POST") {
        const payload = (await request.json()) as any;
        let reqDriverId = payload.driverId;
        let targetDrv = db.drivers.find(
          (d: any) => d.user_id === user.id || d.id === user.id,
        );
        if (!targetDrv && reqDriverId) {
          targetDrv = db.drivers.find((d: any) => d.id === reqDriverId);
        }
        if (!targetDrv)
          return buildResponse({ error: "Driver profile not found." }, 404);
        ...
        let loc = db.driver_locations.find(
          (l: any) => l.driver_id === targetDrv.id,
        );
        if (!loc) {
          loc = {
            id: `LOC-${targetDrv.id}`,
            driver_id: targetDrv.id,
            driver_name:
              targetDrv.full_name ||
              targetDrv.fullName ||
              user.full_name ||
              "Driver",
            company_driver_id: targetDrv.company_driver_id || "DRV-UNKNOWN",
            ...
            activity:
              activity || (speed > 5 ? "In Transit" : "Stationary Work"),
            updated_at: nowIso,
            history: [],
          };
          db.driver_locations.push(loc);
        ...
        saveDB(db);
        return buildResponse({ success: true, location: loc });
      }

      // GET /api/driver/:id/telematics
      if (parts.length === 2 && parts[1] === "telematics" && method === "GET") {
        let targetDriverId = parts[0];
        if (user.role === "driver") {
        ...
        const targetDrv = db.drivers.find(
          (d: any) => d.id === targetDriverId || d.user_id === targetDriverId,
        );
        if (!targetDrv)
          return buildResponse(
            { error: "Driver telematics profile not found." },
            404,
          );

        const dutySessions = (db.driver_duty_sessions || []).filter(
          (s: any) => s.driver_id === targetDrv.id,
        );
        const activeDuty =
          dutySessions.find((s: any) => s.status === "active") || null;
        const currentLocation =
          (db.driver_locations || []).find(
            (l: any) => l.driver_id === targetDrv.id,
          ) || null;

        const maiduguriSim = getMaiduguriSimulatedDataCF(targetDrv);
        ...
        if (!curLoc) {
          curLoc = {
            id: `LOC-${targetDrv.id}`,
            driver_id: targetDrv.id,
            driver_name: targetDrv.fullName || targetDrv.full_name || "Driver",
            company_driver_id: targetDrv.company_driver_id || "DRV-UNKNOWN",
        ...
        return buildResponse({
          ...
          companyDriverId: targetDrv.company_driver_id || "DRV-UNKNOWN",
          activeDuty,
      ...
      if (parts.length === 0 && method === "POST") {
        ...
        try {
          ...
          writeAuditLog(
            ...
            "VEHICLE_ADDED",
            null,
            `Vehicle added: ${payload.plateNumber}`,
            db,
          );
          await dbManager.saveDB(db);
      ...
      if (parts.length === 1 && method === "PUT") {
        ...
        try {
          ...
          writeAuditLog(
            ...
            "VEHICLE_EDITED",
            null,
            `Vehicle plate ${veh.plate_number} modified`,
            db,
          );
          await dbManager.saveDB(db);
          ...
          return buildResponse({
            success: true,
            message: "Vehicle details modified.",
          });
        } catch (err: any) {
          return buildResponse({ error: err.message }, 500);
        }
    ...
    if (path.startsWith("/api/payments")) {
      ...
      if (parts.length === 0 && method === "GET") {
        const dId =
          url.searchParams.get("driverId") || url.searchParams.get("driver_id");
        let payments = db.driver_payments || [];
        if (dId) {
          const drv = (db.drivers || []).find(
            (d: any) =>
              d.id === dId || d.user_id === dId || d.company_driver_id === dId,
          );
          const validIds = new Set(
            [
              dId,
        ...
        try {
          ...
          const newPayment = {
            ...
            receipt_number:
              ...
              `RCP-${generateUUID().substring(0, 6).toUpperCase()}`,
            receiptNumber:
              payload.receiptNumber ||
              payload.referenceNumber ||
              `RCP-${generateUUID().substring(0, 6).toUpperCase()}`,
          ...
          if (targetDrv && initialStatus === "approved") {
            ...
            db.financial_records.push({
              ...
              description: `Installment Payment Approved - Driver ${targetDrv.company_driver_id || "unassigned"} (Receipt: ${newPayment.receipt_number})`,
          ...
          writeAuditLog(
            ...
            "PAYMENT_SUBMITTED",
            null,
            `Payment submitted: ₦${payload.amount}`,
            db,
          );
          await dbManager.saveDB(db);
      ...
      if (parts.length === 2 && parts[1] === "status" && method === "PUT") {
        ...
        try {
          ...
          if (status === "approved" && oldStatus !== "approved") {
            ...
            // Update driver remaining vehicle balance upon admin approval using centralized financial logic
            const drv = db.drivers.find(
              (d: any) => d.id === pay.driver_id || d.user_id === pay.driver_id,
            );
            if (drv) {
            ...
            // Revert remaining balance using centralized financial logic
            const drv = db.drivers.find(
              (d: any) => d.id === pay.driver_id || d.user_id === pay.driver_id,
            );
            if (drv) {
            ...
            db.financial_records = (db.financial_records || []).filter(
              (f: any) => !f.description.includes(pay.receipt_number),
            );
          }
          ...
          db.notifications.unshift({
            id: generateUUID(),
            user_id: db.drivers.find((d: any) => d.id === pay.driver_id)
              ?.user_id,
      ...
      if (parts.length === 1 && method === "PUT") {
        try {
          ...
          if (payload.amount !== undefined)
            pay.amount = parseFloat(payload.amount);
          ...
          const drv = db.drivers.find((d: any) => d.id === pay.driver_id);
          if (drv) {
            const fin = getDriverFinancials(drv, db);
            drv.vehicle_purchase_price = fin.vehiclePurchasePrice;
            drv.vehiclePurchasePrice = fin.vehiclePurchasePrice;
      ...
      if (method === "POST") {
        ...
        try {
          ...
          writeAuditLog(
            ...
            user.email,
            user.role,
            "FINANCE_RECORD_POSTED",
            null,
            `Recorded ${type}: ₦${parsedAmount}`,
            db,
          );
          await dbManager.saveDB(db);
    ...
    if (path === "/api/expenses" && method === "POST") {
      ...
      try {
        ...
        writeAuditLog(
          ...
          user.email,
          user.role,
          "FINANCE_EXPENSE_POSTED",
          null,
          `Recorded expense: ₦${parsedAmount}`,
          db,
        );
        await dbManager.saveDB(db);
    ...
    if (path === "/api/documents/upload-company" && method === "POST") {
      if (user.role !== "admin" && user.role !== "director")
        return buildResponse({ error: "Access Denied." }, 403);
      try {
        const { title, docType, fileBase64, driverId, vehicleId } =
          (await request.json()) as any;
    ...
    if (path.startsWith("/api/documents/preview/")) {
      ...
      const authSession = db.sessions.find(
        (s: any) => s.token === tokenParam && s.status === "active",
      );
      ...
      if (
        ...
        const hasActiveSession =
          db.sessions && db.sessions.some((s: any) => s.status === "active");
      ...
      try {
        ...
        console.error(
          `[PREVIEW FIRESTORE FALLBACK ERROR] Failed to fetch ${filename} from Firestore:`,
          fsErr,
        );
      }
      ...
      if (statusParam) {
        if (statusParam === "unread") {
          enriched = enriched.filter(
            (n: any) => n.status === "unread" || n.read_status === 0,
          );
        } else if (statusParam === "read") {
          enriched = enriched.filter(
            (n: any) => n.status === "read" || n.read_status === 1,
          );
        } else {
    ...
    if (path === "/api/notifications/settings" && method === "POST") {
      try {
        const body = (await request.json()) as any;
        if (!db.user_preferences) db.user_preferences = [];
        let prefsIdx = db.user_preferences.findIndex(
        ...
        const updatedPrefs = {
          ...
          enableSound:
            body.enableSound !== undefined ? !!body.enableSound : true,
          enableVibration:
            body.enableVibration !== undefined ? !!body.enableVibration : true,
          enableAnnouncement:
            body.enableAnnouncement !== undefined
              ? !!body.enableAnnouncement
              : true,
          enableFinanceAlerts:
            body.enableFinanceAlerts !== undefined
              ? !!body.enableFinanceAlerts
              : true,
          enableSecurityAlerts:
            body.enableSecurityAlerts !== undefined
              ? !!body.enableSecurityAlerts
              : true,
    ...
    if (path === "/api/notifications/subscribe" && method === "POST") {
      try {
        ...
        if (env.PUSH_SUBSCRIPTIONS) {
          ...
          return buildResponse({
            success: true,
            message: "KV binding missing but subscription parsed.",
          });
        }
      } catch (err: any) {
        return buildResponse({ error: err.message }, 500);
    ...
    if (path === "/api/notifications/unsubscribe" && method === "POST") {
      try {
        ...
        writeAuditLog(
          ...
          null,
          `User unsubscribed endpoint: ${endpoint.substring(0, 50)}...`,
          db,
        );
        await dbManager.saveDB(db);
        ...
        if (env.PUSH_SUBSCRIPTIONS) {
          ...
          return buildResponse({
            success: true,
            message: "Unsubscribed successfully from DB & KV.",
          });
        } else {
          return buildResponse({
            success: true,
            message: "Unsubscribed successfully from D1 DB.",
          });
        }
      } catch (err: any) {
        return buildResponse({ error: err.message }, 500);
    ...
    if (path === "/api/notifications/status" && method === "GET") {
      ...
      if (env.PUSH_SUBSCRIPTIONS) {
        try {
          const listResult = await env.PUSH_SUBSCRIPTIONS.list();
          const userSubs = (listResult.keys || []).filter((k: any) =>
            k.name.startsWith(`sub:${user.id}:`),
          );
      ...
      if (db.push_subscriptions && Array.isArray(db.push_subscriptions)) {
        const userD1Subs = db.push_subscriptions.filter(
          (s: any) => s && s.userId === user.id,
        );
    ...
    if (path === "/api/notifications/send" && method === "POST") {
      try {
        if (user.role !== "director" && user.role !== "admin") {
          return buildResponse(
            { error: "Unauthorized. Admins or Directors only." },
            403,
          );
        }
        ...
        if (targetUserIds.length === 0) {
          return buildResponse(
            { error: "No recipients matched criteria." },
            404,
          );
        }
        ...
        writeAuditLog(
          ...
          user.email,
          user.role,
          "NOTIFICATION_MANUAL_SEND",
          null,
          `Manual send to ${targetUserIds.length} users`,
          db,
        );
    ...
    if (
      ...
      writeAuditLog(
        ...
        user.email,
        user.role,
        "NOTIFICATION_PIN_TOGGLE",
        id,
        `Toggled pinned status to ${notification.status}`,
        db,
      );
    ...
    if (
      ...
      writeAuditLog(
        ...
        user.email,
        user.role,
        "NOTIFICATION_ARCHIVE_TOGGLE",
        id,
        `Toggled archive status to ${notification.status}`,
        db,
      );
      return buildResponse({ success: true, status: notification.status });
    }
    ...
    if (path === "/api/notifications/history" && method === "GET") {
      ...
      const logsList = (db.audit_logs || []).filter(
        (log: any) => log.action && log.action.startsWith("NOTIFICATION_"),
      );
    ...
    if (path === "/api/notifications/translate" && method === "POST") {
      try {
        ...
        if (to !== "en" && to !== "ha") {
          return buildResponse(
            { error: "Target language must be en or ha." },
            400,
          );
        }
    ...
    if (path === "/api/notifications/bulk" && method === "POST") {
      try {
        ...
        if (!Array.isArray(ids) || ids.length === 0 || !action) {
          return buildResponse(
            { error: "IDs array and action type are required." },
            400,
          );
        }
        ...
        if (updatedCount > 0) {
          ...
          writeAuditLog(
            ...
            user.email,
            user.role,
            `NOTIFICATION_BULK_${action.toUpperCase()}`,
            null,
    ...
    if (path.startsWith("/api/ai/")) {
      ...
      if (path === "/api/ai/chat") {
        ...
        if (!reqPrompt)
          return buildResponse({ error: "Prompt is required." }, 400);
        prompt = reqPrompt;
        history = reqHistory;
...
`;
        ...
        if (!reportType)
          return buildResponse({ error: "Report type is required." }, 400);
        systemPrompt = `You are Ruqayya AI, the highly sophisticated Staff AI Systems Architect and Operations Assistant for RUQAYYA Transport ERP.
Your task is to assist the user by providing accurate, clear, and secure analysis, reporting, searching, or translation.
...
`;
        ...
        const { query } = body;
        if (!query)
          return buildResponse({ error: "Search query is required." }, 400);
        systemPrompt = `You are Ruqayya AI, the highly sophisticated Staff AI Systems Architect and Operations Assistant for RUQAYYA Transport ERP.
Your task is to assist the user by providing accurate, clear, and secure analysis, reporting, searching, or translation.
...
`;
        ...
        if (!documentId)
          return buildResponse({ error: "Document ID is required." }, 400);
        systemPrompt = `You are Ruqayya AI, the highly sophisticated Staff AI Systems Architect and Operations Assistant for RUQAYYA Transport ERP.
Your task is to assist the user by providing accurate, clear, and secure analysis, reporting, searching, or translation.
...
`;
        ...
        if (!entityId)
          return buildResponse(
            { error: "Entity/Transaction ID is required." },
            400,
          );
        systemPrompt = `You are Ruqayya AI, the highly sophisticated Staff AI Systems Architect and Operations Assistant for RUQAYYA Transport ERP.
Your task is to assist the user by providing accurate, clear, and secure analysis, reporting, searching, or translation.
...
`;
              ...
              const functionDeclarations = [
                {
                  ...
                  description:
                    "Record a driver paying an installment. This records their payment against the cycle. Optionally log an expense.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      ...
                      expense_amount: {
                        type: Type.NUMBER,
                        description:
                          "Optional expense amount (e.g. repairs/fines)",
                      },
                      expense_category: {
                        type: Type.STRING,
                ...
                },
                {
                  ...
                  description:
                    "Gets the live remaining balance, installments status, and total purchase amount for a specific driver.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      driver_id: { type: Type.STRING },
                ...
                for (const call of calls) {
                  if (call.name === "record_driver_installment") {
                    ...
                    const driver = db.drivers.find(
                      (d: any) =>
                        ...
                        (d.fullName &&
                          d.fullName
                            .toLowerCase()
                            .includes(args.driver_id.toLowerCase())),
                    );
                    if (!driver) {
                    ...
                    db.driver_payments.push({
                      ...
                      receiptNumber:
                        "AI-REC-" + Math.floor(Math.random() * 10000),
                      status: "approved",
                    ...
                    db.financial_records.push({
                      ...
                      description:
                        "Payment by " + (driver.fullName || driver.id),
                      date: new Date().toISOString().split("T")[0],
                      source: "AI Assistant",
                    ...
                    if (
                      ...
                      db.financial_records.push({
                        ...
                        description:
                          args.expense_description ||
                          "Expense for " + driver.fullName,
                        date: new Date().toISOString().split("T")[0],
                        source: "AI Assistant",
                        cycle_id: args.cycle_id,
                      });
                    ...
                    const totalPaid = db.driver_payments
                      .filter(
                        (p: any) =>
                          p.driver_id === driver.id && p.status === "approved",
                      )
                      .reduce(
                    ...
                    const newBalance = Math.max(
                      0,
                      (Number(driver.agreedAmount) || 0) - totalPaid,
                    );
    ...
    if (path.startsWith("/api/shareholders")) {
      ...
      if (parts.length === 0) {
        ...
        if (method === "POST") {
          ...
          try {
            ...
            const rawAmount =
              payload.investmentAmount !== undefined
            ...
            const investmentDate =
              ...
              new Date().toISOString().split("T")[0];
            const passportPhoto =
              payload.passportPhoto || payload.passport_photo_url || "";
            ...
            if (
              db.shareholders.some(
                (s: any) => s.email && s.email.toLowerCase() === email,
              )
            ) {
              return buildResponse(
                { error: "Email registered to another investor node." },
                400,
              );
            }
            ...
            const hashedShareholderPassword = await hashPassword(
            ...
            writeAuditLog(
              ...
              "SHAREHOLDER_ADDED",
              null,
              `Created shareholder: ${payload.full_name}`,
              db,
            );
            await dbManager.saveDB(db);
      ...
      if (parts.length === 2 && parts[1] === "archive" && method === "PUT") {
        try {
          ...
          if (!sh)
            return buildResponse({ error: "Shareholder not found." }, 404);
          sh.status = "archived";
          await dbManager.saveDB(db);
          return buildResponse({
            success: true,
            message: "Shareholder archived",
          });
        } catch (err: any) {
          return buildResponse({ error: err.message }, 500);
        }
      ...
      if (parts[0] === "me" && method === "GET") {
        const sh = db.shareholders.find((s: any) => s.user_id === user.id);
        if (!sh)
          return buildResponse({ error: "Shareholder profile missing." }, 404);
        ...
        const equityPercentage =
          totalInvested > 0
            ? (
                ((parseFloat(sh.investment_amount) || 0) / totalInvested) *
                100
              ).toFixed(2)
            : "0";
      ...
      if (parts[0] === "self" && method === "PUT") {
        ...
        try {
          ...
          if (email) {
            ...
            if (emailExists)
              return buildResponse({ error: "Email already registered." }, 400);
          ...
          writeAuditLog(
            ...
            user.email,
            user.role,
            "SHAREHOLDER_SELF_UPDATE",
            null,
            "Shareholder updated self profile",
            db,
          );
          await dbManager.saveDB(db);
      ...
      if (parts.length === 1) {
        ...
        if (method === "PUT") {
          ...
          try {
            ...
            if (payload.investment_amount !== undefined)
              sh.investment_amount = parseFloat(payload.investment_amount);
            if (payload.passport_photo_url !== undefined)
              sh.passport_photo_url = payload.passport_photo_url;
            if (payload.passport_number !== undefined)
              sh.passport_number = payload.passport_number;
            ...
            writeAuditLog(
              ...
              "SHAREHOLDER_EDITED",
              null,
              `Modified shareholder: ${sh.full_name}`,
              db,
            );
            await dbManager.saveDB(db);
        ...
        if (method === "DELETE") {
          if (user.role !== "director")
            return buildResponse(
              { error: "Access Denied: Board level authorization required." },
              403,
            );
          db.shareholders = db.shareholders.filter(
            (s: any) => s.id !== parts[0],
          );
          writeAuditLog(
            user.id,
            ...
            "SHAREHOLDER_DELETED",
            null,
            `Removed shareholder: ${sh.full_name}`,
            db,
          );
          await dbManager.saveDB(db);
          return buildResponse({ success: true });
    ...
    if (path.startsWith("/api/trips")) {
      ...
      if (parts.length === 0) {
        if (method === "GET") {
          const list = (db.trip_manifests || []).map((t: any) => ({
            ...
            destination:
              t.destination ||
              (t.origin ? `Trip from ${t.origin}` : "Operational Route"),
            departureTime:
              t.departure_time ||
              t.departureTime ||
              t.start_date ||
            ...
            departure_time:
              ...
              t.created_at,
            expectedArrivalTime:
              t.expected_arrival_time || t.expectedArrivalTime,
        ...
        if (method === "POST") {
          ...
          try {
            ...
            writeAuditLog(
              ...
              "TRIP_CREATED",
              null,
              `Trip ${newTrip.trip_number} created`,
              db,
            );
            await dbManager.saveDB(db);
      ...
      if (parts.length === 2 && parts[1] === "complete" && method === "PUT") {
        ...
        try {
          ...
          if (!trip)
            return buildResponse({ error: "Trip record not found." }, 404);
    ...
    if (path.startsWith("/api/director/")) {
      ...
      if (!isPublicCyclesGet) {
        if (user.role !== "director" && user.role !== "admin") {
          return buildResponse(
            {
              error:
                "Access Denied: Executive Director or Admin privileges required.",
            },
            403,
          );
        }
      ...
      if (ctrl === "cycles/start" && method === "POST") {
        try {
          ...
          writeAuditLog(
            ...
            "CYCLE_STARTED",
            null,
            `Started operating cycle: ${newCycle.id}`,
            db,
          );
          await dbManager.saveDB(db);
      ...
      if (ctrl === "cycles/pause" && method === "POST") {
        try {
          ...
          if (!reason) {
            return buildResponse(
              { error: "Reason for pause is required." },
              400,
            );
          }
          ...
          if (!activeCycle) {
            return buildResponse(
              { error: "No active operating cycle found to pause." },
              400,
            );
          }
          ...
          const newExtendedDays =
            (activeCycle.extendedDays || 0) + daysToExtend;
          ...
          activeCycle.endDate = new Date(extendedEndMs)
            .toISOString()
            .split("T")[0];
      ...
      if (ctrl === "cycles/resume" && method === "POST") {
        try {
          ...
          if (!pausedCycle) {
            return buildResponse(
              { error: "No paused operating cycle found to resume." },
              400,
            );
          }
      ...
      if (ctrl === "cycles/end" && method === "POST") {
        try {
          ...
          if (!activeCycle)
            return buildResponse(
              { error: "No active operating cycle found." },
              404,
            );
          ...
          writeAuditLog(
            ...
            "CYCLE_ENDED",
            null,
            `Closed cycle: Net pool distributed ₦${distributionPool}`,
            db,
          );
          await dbManager.saveDB(db);
      ...
      if (ctrl === "shareholders" && method === "PUT") {
        return buildResponse(
          { error: "Use /api/shareholders/:id endpoints directly" },
          400,
        );
      }
      ...
      if (
        ctrl.startsWith("shareholders/") &&
        ctrl.endsWith("/status") &&
        method === "PUT"
      ) {
        try {
        ...
        try {
          ...
          if (sh) {
            ...
            writeAuditLog(
              ...
              id,
              `Investment updated to ${total_investment}`,
              db,
            );
            await dbManager.saveDB(db);
            return buildResponse({ success: true });
      ...
      if (ctrl === "shareholder-settings" && method === "PUT") {
        try {
          ...
          writeAuditLog(
            ...
            null,
            `Dividend distribution set to ${distributionPercentage}%`,
            db,
          );
          await dbManager.saveDB(db);
          return buildResponse({ success: true });
      ...
      if (ctrl === "company-settings" && method === "PUT") {
        try {
          ...
          writeAuditLog(
            ...
            null,
            `Updated general corporate configuration`,
            db,
          );
          await dbManager.saveDB(db);
          return buildResponse({ success: true });
      ...
      if (ctrl.startsWith("admins/") && method === "PUT") {
        try {
          ...
          if (admin) {
            ...
            if (updates.password)
              admin.password_hash = await generateHash(updates.password);
      ...
      if (
        ctrl.startsWith("drivers/") &&
        ctrl.endsWith("/add-accident") &&
        method === "POST"
      ) {
        try {
          ...
          const { date, description, damageEstimate, severity } =
            (await request.json()) as any;
          if (!date || !description)
            return buildResponse(
              { error: "Date and description parameters are required." },
              400,
            );
      ...
      if (
        ...
        try {
          ...
          writeAuditLog(
            ...
            user.email,
            user.role,
            "DRIVER_REST_PERIOD_SCHEDULED",
            null,
            `Scheduled rest period for driver: ${driver.id} (${startDate} to ${endDate})`,
            db,
          );
          await dbManager.saveDB(db);
      ...
      if (ctrl.startsWith("directors/") && method === "PUT") {
        try {
          ...
          if (dirUser) {
            ...
            if (updates.password)
              dirUser.password_hash = await generateHash(updates.password);
    ...
    if (path === "/api/directory/all" && method === "GET") {
      if (user.role !== "admin" && user.role !== "director") {
        return buildResponse(
          {
            error:
              "Access Denied: Administrative or Board credentials required.",
          },
          403,
        );
      }
      try {
        const drivers = (db.drivers || []).map((drv: any) => {
          const u = (db.users || []).find(
            (userObj: any) => userObj.id === drv.user_id,
          );
          const g = (db.guarantors || []).find(
            (gua: any) => gua.driver_id === drv.id,
          );
          const v = (db.vehicles || []).find(
            (veh: any) => veh.driver_id === drv.id,
          );
          const financials = getDriverFinancials(drv, db);
          ...
          return {
            ...
            status: drv.status,
            registrationDate:
              drv.created_at || u?.created_at || new Date().toISOString(),
            guarantor: g,
            vehicle: v,
        ...
        const shareholders = (db.shareholders || []).map((sh: any) => {
          ...
          const fundedDrivers = (db.drivers || [])
            ...
            .map((d: any) => {
              const u = (db.users || []).find(
                (userObj: any) => userObj.id === d.user_id,
              );
        ...
        const admins = (db.admins || []).map((adm: any) => {
          const u = (db.users || []).find(
            (userObj: any) => userObj.id === adm.user_id,
          );
          ...
          return {
            ...
            registrationDate:
              adm.created_at || u?.created_at || new Date().toISOString(),
        ...
        const directors = (db.directors || []).map((dir: any) => {
          const u = (db.users || []).find(
            (userObj: any) => userObj.id === dir.user_id,
          );
          const signaturesCount = (db.audit_logs || []).filter(
            (l: any) =>
              l.userId === dir.user_id && l.action.includes("APPROVED"),
          ).length;
          return {
            ...dir,
            ...
            registrationDate:
              dir.created_at || u?.created_at || new Date().toISOString(),
    ...
    if (path === "/api/operations/state" && method === "GET") {
      try {
        ...
        if (
          companyWalletBalance === undefined ||
          companyWalletBalance === null
        ) {
          const initial =
            parseFloat(db.company_settings?.wallet_initial_amount) || 0;
    ...
    if (path === "/api/operations/start" && method === "POST") {
      ...
      try {
        ...
        if (state.status !== "Setup Mode") {
          return buildResponse(
            { error: "Company operations have already been initialized." },
            400,
          );
        }
        ...
        writeAuditLog(
          ...
          "Setup Mode",
          `Activated live enterprise operations. First 30-day operating cycle commenced by ${user.fullName}`,
          db,
        );
        await dbManager.saveDB(db);
        ...
        return buildResponse({
          ...
          state: updatedState,
          detail: generateFilteredPayload(
            user.role,
            driverProfileId,
            shareholderId,
            db,
    ...
    if (path === "/api/operations/pause" && method === "POST") {
      ...
      try {
        ...
        if (!reason) {
          return buildResponse(
            { error: "Reason for suspension is mandatory." },
            400,
          );
        }
        ...
        if (state.status !== "Operational Mode") {
          return buildResponse(
            { error: "Operations can only be paused from Operational Mode." },
            400,
          );
        }
        ...
        if (activeCycle) {
          ...
          const startMs = activeCycle.startDate
            ? new Date(
                activeCycle.startDate.includes("T")
                  ? activeCycle.startDate
                  : `${activeCycle.startDate}T00:00:00`,
              ).getTime()
    ...
    if (path === "/api/operations/resume" && method === "POST") {
      ...
      try {
        ...
        if (state.status !== "Paused") {
          return buildResponse(
            { error: "Operations can only be resumed when Paused." },
            400,
          );
        }
    ...
    if (path === "/api/operations/config-salaries" && method === "POST") {
      ...
      try {
        ...
        if (!salaries || !Array.isArray(salaries)) {
          return buildResponse(
            { error: "Invalid salary configurations payload." },
            400,
          );
        }
    ...
    if (path === "/api/finance/withdraw" && method === "POST") {
      ...
      try {
        const { shareholderId, amount, remarks } =
          (await request.json()) as any;
        if (!shareholderId || !amount || parseFloat(amount) <= 0) {
          return buildResponse(
            { error: "Invalid withdrawal amount or shareholder ID." },
            400,
          );
        }
        ...
        const pctStake =
          totalInvestmentsSum > 0
            ? ((parseFloat(sh.investment_amount) || 0) / totalInvestmentsSum) *
              100
            : 0;
        ...
        if (withdrawAmt > availableWithdrawal) {
          return buildResponse(
            {
              error: `Over-withdrawal prevented. Maximum available: ₦${availableWithdrawal.toLocaleString()}`,
            },
            400,
          );
        }
        ...
        sh.total_withdrawn =
          parseFloat(totalWithdrawn) + parseFloat(withdrawAmt);
        sh.updated_at = new Date().toISOString();
    ...
    if (path === "/api/finance/reinvest" && method === "POST") {
      ...
      try {
        ...
        if (!shareholderId || !amount || parseFloat(amount) <= 0) {
          return buildResponse(
            { error: "Invalid reinvestment amount or shareholder ID." },
            400,
          );
        }
        ...
        if (reinvestAmt > availableWithdrawal) {
          return buildResponse(
            {
              error: `Over-reinvestment prevented. Maximum available: ₦${availableWithdrawal.toLocaleString()}`,
            },
            400,
          );
        }
        ...
        sh.total_withdrawn =
          parseFloat(totalWithdrawn) + parseFloat(reinvestAmt);
        sh.updated_at = new Date().toISOString();
      ...
      if (method === "POST") {
        try {
          ...
          if (!receiverId || !receiverRole) {
            return buildResponse(
              { error: "Receiver id and role parameters required." },
              400,
            );
          }
    ...
    if (path === "/api/announcements") {
      ...
      if (method === "POST") {
        if (user.role !== "admin" && user.role !== "director") {
          return buildResponse(
            { error: "Access Denied: Admins or Directors only." },
            403,
          );
        }
        try {
    ...
    if (path === "/api/auth/register-admin" && method === "POST") {
      if (user.role !== "director" && user.role !== "admin") {
        return buildResponse(
          { error: "Access Denied: Directors-only credential endpoint." },
          403,
        );
      }
      try {
    ...
    if (path === "/api/auth/change-password-first-login" && method === "POST") {
      try {
        ...
        const roleName =
          db.roles.find((r: any) => r.id === userRec.role_id)?.name ||
          user.role ||
          "public";
        return buildResponse({
          success: true,
    ...
    if (path === "/api/finance/cap-out" && method === "POST") {
      try {
        ...
        if (user.role === "shareholder") {
          ...
          if (!sh)
            return buildResponse(
              { error: "Shareholder profile not found." },
              404,
            );
          ...
          if (!shareholderId)
            return buildResponse({ error: "Shareholder ID required." }, 400);
        ...
        sh.total_cashed_out =
          (parseFloat(sh.total_cashed_out) || 0) + capOutAmt;
        sh.updated_at = new Date().toISOString();
        ...
        writeAuditLog(
          ...
          "SHAREHOLDER_CAPOUT",
          null,
          `Shareholder ${sh.full_name} capital redemption of ₦${capOutAmt.toLocaleString()}`,
          db,
        );
    ...
    if (path === "/api/documents/replace" && method === "POST") {
      ...
      try {
        const { docId, category, title, fileBase64 } =
          (await request.json()) as any;
        if (!docId || !category || !fileBase64) {
          return buildResponse(
            { error: "Missing mandatory replacement arguments." },
            400,
          );
        }
        ...
        const docTitle =
          title || doc.title || doc.document_type || "Replaced_Doc";
        const fileId = `${Date.now()}-${generateUUID().substring(0, 8)}`;
        const filename = `${fileId}.png`;
        ...
        const accounts = (db.users || []).map((u: any) => {
          ...
          if (role === "driver") {
            const d = (db.drivers || []).find(
              (driver: any) =>
                driver.user_id === u.id || driver.id === u.driver_id,
            );
            if (d) {
              profileInfo = {
    ...
    if (
      ...
      try {
        ...
        if (userIndex === -1) {
          return buildResponse(
            { error: "Target user account not found in system directory." },
            404,
          );
        }
        ...
        if (
          targetUser.role_id === "role-driver" ||
          targetUser.role === "driver"
        ) {
          const driver = (db.drivers || []).find(
            (d: any) =>
              d.user_id === targetUser.id || d.id === targetUser.driver_id,
          );
          if (driver) {
          ...
          targetUser.role_id === "role-shareholder" ||
          targetUser.role === "shareholder"
        ) {
          const shareholder = (db.shareholders || []).find(
            (s: any) =>
              s.user_id === targetUser.id || s.id === targetUser.shareholder_id,
          );
          if (shareholder) {
            if (nameToSet !== undefined)
              shareholder.full_name = nameToSet.trim();
        ...
        writeAuditLog(
          ...
          "CREDENTIAL_UPDATE",
          userId,
          `Updated credentials/status for user ID: ${userId} (${targetUser.username})`,
          db,
        );
    ...
    if (path === "/api/admin/reset-test-data" && method === "POST") {
      ...
      try {
        ...
        db.admins = (db.admins || []).filter((a: any) =>
          keptUserIds.has(a.user_id),
        );
        db.directors = (db.directors || []).filter((d: any) =>
          keptUserIds.has(d.user_id),
        );
    ...
    return buildResponse(
      { error: `The requested corporate endpoint ${path} is non-existent.` },
      404,
    );
  } catch (err: any) {
