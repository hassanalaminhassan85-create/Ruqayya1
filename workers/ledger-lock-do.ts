...
export class LedgerLock {
  ...
  async handleDriverPayment(payload: any) {
    ...
    const paymentId = `PAY-${Date.now()}-${this.cryptoRandom(6)}`;
    ...
    if (!driverInsts || driverInsts.length === 0) {
      const gen = this.generateSixInstallments(driver_id, payload.cycle_id || `cycle-${Date.now()}`, payload.agreed_amount_30 || (driver.agreed_amount_30 || 180000), payload.cycle_start_date);
    ...
    if (remaining > 0) {
      fin_records.unshift({ id: `LEDGER-${Date.now()}-${this.cryptoRandom(4)}`, type: 'revenue', category: 'overpayment', amount: remaining, date: new Date().toISOString().split('T')[0], description: `Overpayment credit from ${paymentId}` });
      fin_ledger.unshift({ id: `LEDGER-${Date.now()}-${this.cryptoRandom(4)}`, type: 'revenue', subtype: 'overpayment_credit', amount: remaining, created_at: nowIso(), driver_id, reference_id: paymentId, description: 'overpayment credit' });
    }

    // Create ledger entry for the payment (full amount)
    const ledgerId = `LEDGER-${Date.now()}-${this.cryptoRandom(4)}`;
    ...
    // Audit log entry
    const audit = { id: `AUD-${Date.now()}-${this.cryptoRandom(4)}`, user_id: payload.actor || null, action: 'DRIVER_PAYMENT', previous_value: null, new_value: { paymentId, driver_id, amount }, created_at: nowIso() };
  ...
  async handleRecordExpense(payload: any) {
    ...
    const id = `EXP-${Date.now()}-${this.cryptoRandom(4)}`;
    ...
    const audit = { id: `AUD-${Date.now()}-${this.cryptoRandom(4)}`, user_id: payload.actor || null, action: 'RECORD_EXPENSE', previous_value: null, new_value: rec, created_at: nowIso() };
    audit_logs.unshift(audit);
  ...
  async handleGenerateInstallments(payload: any) {
    ...
    const installments = (await this.readCollection('installments')) || [];
    const generated = this.generateSixInstallments(driver_id, cycle_id, agreed_amount_30, payload.cycle_start_date);
    for (const g of generated) installments.unshift(g);
    await this.writeCollection('installments', installments);

    const audit_logs = (await this.readCollection('audit_logs')) || [];
    const audit = { id: `AUD-${Date.now()}-${this.cryptoRandom(4)}`, user_id: payload.actor || null, action: 'GENERATE_INSTALLMENTS', previous_value: null, new_value: { driver_id, cycle_id, count: generated.length }, created_at: nowIso() };
    audit_logs.unshift(audit);
    await this.writeCollection('audit_logs', audit_logs);
  ...
  // small helpers --------------------------------------------------------------
  cryptoRandom(n = 6) {
    // basic secure random hex using Web Crypto
    const arr = new Uint8Array(n);
    crypto.getRandomValues(arr as any);
    return Array.from(arr).map((b: number) => b.toString(16).padStart(2, '0')).join('').slice(0, n).toUpperCase();
  }

  generateSixInstallments(driver_id: string, cycle_id: string, agreed_amount_30: number, cycle_start_date?: string) {
