...
async function runCycleUpdates(env: Env) {
  ...
  let summaryStats = {
    ...
    processedVehicles: 0,
    expiringDocumentsFlagged: 0,
    maintenanceFlagsSet: 0,
    cycleClosed: false
  };
  ...
  if (activeCycle) {
    ...
    // 3. Driver payments calculations, overdue detection, penalties, and upcoming reminders
    console.log("Cron: Assessing driver payment installments for overdue items and upcoming due dates...");
    for (const driver of db.drivers) {
      const status = driver.status;
      if (status !== 'approved' && status !== 'active') continue;

      summaryStats.processedDrivers++;
      ...
      for (const inst of installments) {
        ...
        // TASK 2: Overdue detection and applying penalty
        if (inst.status === 'Overdue') {
          summaryStats.overdueInstallments++;

          // Initialize penalties history structure if absent
          if (!driver.penalties_history) {
            driver.penalties_history = [];
          }

          // Check if already penalized for this specific installment inside this cycle
          const penaltyAlreadyApplied = driver.penalties_history.some((p: any) => 
            p.installmentNumber === inst.installmentNumber && 
            p.cycleId === activeCycle.id
          );

          if (!penaltyAlreadyApplied) {
            const penaltyFee = 5000; // Overdue charge penalty of ₦5,000
            console.log(`Cron: Applying ₦5,000 penalty to Driver ${driver.fullName || driver.id} for installment #${inst.installmentNumber}`);
            
            // Add to driver penalty and debt records
            driver.total_penalty_amount = (driver.total_penalty_amount || 0) + penaltyFee;
            driver.debt_amount = (driver.debt_amount || 0) + penaltyFee;
            ...
            // Register penalty accrued as a financial entry
            db.financial_records.push({
              id: generateUUID(),
              type: 'revenue',
              category: 'penalty',
              ...
              description: `Auto-Overdue Penalty Applied: Driver ${driver.fullName || 'Candidate'} (Installment #${inst.installmentNumber} in Cycle ${activeCycle.id})`
            });

            const msg_en = `Installment #${inst.installmentNumber} for Driver ${driver.fullName || 'Candidate'} is OVERDUE! A ₦5,000 penalty has been applied to your balance. Outstanding: ₦${(inst.remainingAmount + penaltyFee).toLocaleString()}.`;
            const msg_ha = `Biyan kuɗi kashi na #${inst.installmentNumber} na Direba ${driver.fullName || 'Candidate'} ya wuce lokaci! An tara tarar ₦5,000 akan asusunku. Ragowar kuɗi: ₦${(inst.remainingAmount + penaltyFee).toLocaleString()}.`;
            ...
            writeAuditLog("system", "cron", "DRIVER_PAYMENT_PENALTY", `Applied ₦5,000 penalty to driver ${driver.fullName || driver.id} for installment #${inst.installmentNumber}`, db);
            dbChanged = true;
            summaryStats.penaltiesApplied++;
            summaryStats.totalPenaltiesAmount += penaltyFee;

            // Deliver real-time push warnings
            if (driver.user_id) {
              await sendPushNotificationToUserOrRole(env, { userId: driver.user_id }, {
                title: 'Overdue Penalty Notice',
                message: msg_en,
                type: 'danger'
              });
            }
            await sendPushNotificationToUserOrRole(env, { role: 'admin' }, {
              ...
              message: `Driver ${driver.fullName || driver.id} was charged ₦5,000 overdue penalty on installment #${inst.installmentNumber}.`,
              type: 'warning'
            });
          }
        }

        // TASK 4: Upcoming deadline reminders (due in < 48 hours and is pending/partially paid)
        else if (inst.status !== 'Completed' && hoursRemaining > 0 && hoursRemaining <= 48) {
          // Verify we have not sent a recent notification for this upcoming event
          const reminderAlreadySent = db.notifications.some((n: any) => 
            n.driver_id === driver.id && 
            n.installmentNumber === inst.installmentNumber &&
            n.cycle_id === activeCycle.id &&
            n.type === 'upcoming_reminder' &&
            (new Date().getTime() - new Date(n.created_at).getTime()) < 24 * 3600 * 1000 // Sent within 24 hours
          );

          if (!reminderAlreadySent) {
            console.log(`Cron: Generating upcoming reminder for Driver ${driver.fullName || driver.id} (Installment #${inst.installmentNumber})`);
            
            const msg_en = `Upcoming Reminder: Your installment #${inst.installmentNumber} of ₦${inst.remainingAmount.toLocaleString()} is due on ${inst.endDate}. Please coordinate payment.`;
            const msg_ha = `Gargaɗi na Kusa: Biyan kuɗi kashi na #${inst.installmentNumber} na ₦${inst.remainingAmount.toLocaleString()} zai cika a ranar ${inst.endDate}. Don Allah a shirya biya.`;
            ...
            db.notifications.unshift({
              ...
              installmentNumber: inst.installmentNumber,
              cycle_id: activeCycle.id,
              title_en: 'Upcoming Installment Due',
              title_ha: 'Ranar Biyan Kusa Ta Karaso',
              message_en: msg_en,
              message_ha: msg_ha,
            ...
            dbChanged = true;
            summaryStats.upcomingRemindersSent++;

            // Push notification
            if (driver.user_id) {
              await sendPushNotificationToUserOrRole(env, { userId: driver.user_id }, {
                title: 'Payment Reminder Due Soon',
                message: msg_en,
                type: 'info'
              });
            }
  ...
  // TASK 3: Fleet Status & Expiring Document Updates
  console.log("Cron: Auditing fleet registry and monitoring vehicle indicators...");
  const fleet = db.vehicles || [];
  
  for (const vehicle of fleet) {
    summaryStats.processedVehicles++;
    let vehicleModified = false;

    // 1. Check insurance & registration expirations (flag if expired or due in < 7 days)
    const today = new Date();
    const alertThresholdMs = 7 * 24 * 3600 * 1000; // 7 days

    const docExpiries = [
      { name: 'insurance', value: vehicle.insurance_expiry, desc: 'Insurance policy' },
      { name: 'registration', value: vehicle.registration_expiry, desc: 'Vehicle registration document' },
      { name: 'permit', value: vehicle.permit_expiry, desc: 'Commercial carriage permit' }
    ];

    for (const doc of docExpiries) {
      ...
      const isExpiringSoon = msDiff > 0 && msDiff <= alertThresholdMs;

      if (expired || isExpiringSoon) {
        // Prevent writing duplicate expiring logs within 3 days
        const recentAlert = db.notifications.some((n: any) => 
          n.vehicle_plate === vehicle.plate_number && 
          n.document_type === doc.name &&
          (new Date().getTime() - new Date(n.created_at).getTime()) < 3 * 24 * 3600 * 1000
        );

        if (!recentAlert) {
          const statusText = expired ? 'EXPIRED' : 'EXPIRING SOON';
          const msg_en = `ALERT: ${doc.desc} for vehicle ${vehicle.plate_number} has ${statusText} on ${doc.value}. Please renew immediately.`;
          const msg_ha = `GARGAƊI: ${doc.desc} na abun hawa ${vehicle.plate_number} ya ${expired ? 'KARE' : 'KUSA KAREWA'} a ranar ${doc.value}. Da fatan za a sabunta asusu.`;

          db.notifications.unshift({
            id: generateUUID(),
            vehicle_plate: vehicle.plate_number,
            document_type: doc.name,
            title_en: `Vehicle ${doc.name} ${expired ? 'Expired' : 'Expiring'}`,
            title_ha: `Kari ko karewar takardar ${doc.name}`,
            message_en: msg_en,
            message_ha: msg_ha,
            type: 'warning',
            read_status: 0,
          ...
          dbChanged = true;
          summaryStats.expiringDocumentsFlagged++;

          // Send push to administrators and directors
          await sendPushNotificationToUserOrRole(env, { role: 'admin' }, {
            title: `Fleet Expiry Alert: ${vehicle.plate_number}`,
            message: msg_en,
            type: 'warning'
          });
    ...
    // 2. Mileage-based maintenance/oil change checks
    if (vehicle.current_mileage && vehicle.oil_change_mileage) {
      const current = parseFloat(vehicle.current_mileage);
      const target = parseFloat(vehicle.oil_change_mileage);

      if (current >= target) {
        // Exceeded mileage threshold - trigger maintenance flags
        if (vehicle.status !== 'maintenance' && vehicle.status !== 'maintenance required') {
          console.log(`Cron: Vehicle ${vehicle.plate_number} exceeded oil change mileage. Auto flagging maintenance.`);
          vehicle.status = 'maintenance required';
          vehicleModified = true;
          summaryStats.maintenanceFlagsSet++;

          const msg_en = `Maintenance Required: Vehicle ${vehicle.plate_number} has reached its oil change mileage limit (${current.toLocaleString()} km / Limit: ${target.toLocaleString()} km).`;
          const msg_ha = `Gyara na Dole: Abun hawa ${vehicle.plate_number} ya kai iyakar nisan canza mai (${current.toLocaleString()} km / Iyakar: ${target.toLocaleString()} km).`;

          db.notifications.unshift({
            id: generateUUID(),
            vehicle_plate: vehicle.plate_number,
            title_en: 'Oil Change Maintenance Due',
            title_ha: 'Canza Mai Ya Zama Dole',
            message_en: msg_en,
            message_ha: msg_ha,
            type: 'warning',
            read_status: 0,
          ...
          writeAuditLog("system", "cron", "VEHICLE_MAINTENANCE_DUE", `Flagged vehicle ${vehicle.plate_number} for maintenance. Current: ${current} km, Target: ${target} km.`, db);

          // Alert admins
          await sendPushNotificationToUserOrRole(env, { role: 'admin' }, {
            title: `Vehicle Oil Change Alert: ${vehicle.plate_number}`,
            message: msg_en,
            type: 'warning'
          });
        }
  ...
  // TASK 5: Optionally call Workers AI to summarize operational diagnostics if the AI binding is available
  if (env.AI) {
    try {
      console.log("Cron: Running Workers AI Operational Advisor model...");
      const aiPrompt = `Perform a Staff-level operations health diagnostic of our transport enterprise.
Current diagnostic run completed with these figures:
- Processed Drivers count: ${summaryStats.processedDrivers}
- Overdue payments detected: ${summaryStats.overdueInstallments}
- Automated penalties applied: ${summaryStats.penaltiesApplied} (Totaling: ₦${summaryStats.totalPenaltiesAmount.toLocaleString()})
- Upcoming installment due reminders dispatched: ${summaryStats.upcomingRemindersSent}
- Audited fleet size: ${summaryStats.processedVehicles}
- Pending document expiries flagged: ${summaryStats.expiringDocumentsFlagged}
- Auto-maintenance warnings issued: ${summaryStats.maintenanceFlagsSet}
- Active cycle concluded this run: ${summaryStats.cycleClosed ? "YES" : "NO"}

Generate a highly professional, extremely brief (2-3 sentences max) executive staff recommendation report evaluating the current health status and identifying any critical bottlenecks or operational actions the director should take immediately. Return ONLY the report text.`;

      // Run standard Cloudflare Llama model
      ...
      if (aiResponse && aiResponse.response) {
        const diagnosticStr = aiResponse.response.trim();
        console.log("Cron AI Advisor Output:", diagnosticStr);
        db.company_operations_state.lastAIDiagnostic = diagnosticStr;
        db.company_operations_state.lastAIDiagnosticDate = new Date().toISOString();
        dbChanged = true;

        // Log AI recommendation inside Audit Log
        writeAuditLog("system", "cron", "AI_OPERATIONAL_DIAGNOSTIC", `AI Advisor Recommendation: ${diagnosticStr}`, db);
      }
    } catch (aiErr) {
      console.warn("Cron: Gracefully bypassed AI generation due to run failure:", aiErr);
    }
  }

  // Always write a cron heartbeat audit log to ensure monitoring visibility
  writeAuditLog("system", "cron", "CRON_HEARTBEAT", null, `Cron trigger executed successfully. processedDrivers: ${summaryStats.processedDrivers}, processedVehicles: ${summaryStats.processedVehicles}, penaltiesApplied: ${summaryStats.penaltiesApplied}`, db);
  dbChanged = true;
